...
Genera un archivo SQL para ejecutar con wrangler d1 execute.
Trae resultados "Mayor" desde 2024-01-01 hasta 2025-04-02.

Uso:
  cd api
  python scripts/ingest-historical.py
  npx wrangler d1 execute loter-ia-db --remote --file=scripts/historical_inserts.sql
"""
...
OUTPUT = "scripts/historical_inserts.sql"
BATCH_SIZE = 5000
...
def fetch_data(offset=0):
    params = urllib.parse.urlencode({
        "$limit": BATCH_SIZE,
        "$offset": offset,
        "$where": "tipo_de_premio='Mayor' AND a_o_del_sorteo>='2020'",
...
def main():
    ...
    while True:
        ...
        for row in data:
            ...
            # Solo hasta antes de donde empiezan nuestros datos actuales
            if fecha >= "2025-04-03":
                continue
            ...
            sql = (
                ...
                f"SELECT id, '{fecha}', '{numero_esc}', {serie_val}, {sorteo_val}, 'mayor', 'datos.gov.co' "
